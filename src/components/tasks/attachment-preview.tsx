...
import { useSignedUrl } from "@/hooks/use-attachments";
...
export function AttachmentPreview({ isOpen, onClose, attachment }: AttachmentPreviewProps) {
    ...
    // Get signed URL for FILE types
    const { data: signedUrl, isLoading: isSigning } = useSignedUrl(
        isOpen && attachment?.type === "FILE" ? attachment.id : null
    );

    const effectiveUrl = attachment?.type === "FILE" ? signedUrl : attachment?.url;

    useEffect(() => {
        if (isOpen && attachment && attachment.type === "FILE" && effectiveUrl) {
            const isText = 
    ...
    }, [isOpen, attachment, effectiveUrl]);

    if (!attachment) return null;

    const isLoading = isSigning || (attachment.type === "FILE" && !effectiveUrl) || isFetchingText;
            ...
            <DialogContent className="max-w-4xl w-[90vw] h-[80vh] flex flex-col p-0 gap-0 overflow-hidden bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 shadow-2xl [&>button]:hidden">
                ...
                <DialogHeader className="p-4 border-b flex-row items-center justify-between space-y-0 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div className="flex items-center gap-2">
                        {attachment.type === "FILE" && effectiveUrl && (
                            <a href={effectiveUrl} download={attachment.name}>
                ...
                <div className="flex-1 overflow-hidden bg-slate-50/30 dark:bg-slate-900/10">
                    {isLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center p-8 opacity-40">
                            ...
                            {effectiveUrl && (
                                <a href={effectiveUrl} download={attachment.name} className="mt-4">
