...
import { Clock, User, Mail, Lock, ArrowRight } from "lucide-react";
...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md bg-[#0A0A0B] border-white/10 text-white p-0 overflow-hidden">
                <div className="relative p-8">
                    ...
                    {config.enableCredentialsAuth && (
                        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            className="bg-white/5 border-white/10 pl-10 h-11 focus:ring-primary/20"
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            <Button
                                type="submit"
                                disabled={isLoading}
                                className="w-full h-11 rounded-xl bg-primary text-white font-bold shadow-lg shadow-primary/20 mt-2 group"
                            >
                                {isLoading ? (
                    ...
                    {config.enableGoogleAuth && (
                        <div className={`mt-6 ${config.enableCredentialsAuth ? 'pt-6 border-t border-white/5' : ''} space-y-4`}>
                            <Button
                                onClick={() => signIn("google", { callbackUrl: "/app" })}
                                variant="outline"
                                className="w-full bg-white/5 border-white/10 hover:bg-white/10 text-white h-11 font-bold rounded-xl transition-all"
                            >
                    ...
                    {config.enableCredentialsAuth && (
                        <div className="mt-8 pt-6 border-t border-white/5 text-center">
                            <p className="text-sm text-slate-400 font-medium italic">
                                {isLogin ? "New to FocusOS?" : "Already have a sanctuary?"}{" "}
                                <button
                    ...
                    <p className="mt-6 text-center text-[10px] text-slate-600 font-medium uppercase tracking-widest">
                        FocusOS • AGPLv3 Licensed
                    </p>
                </div>
            </DialogContent>
        </Dialog>
